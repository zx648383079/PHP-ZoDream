        ...
        <?php foreach($model_list as $item):?>
            <tr>
                ...
                <td>
                    <?php if($id < 1):?>
                    <div class="btn-group">
                        <a class="btn btn-default" href="<?=$this->url('./@admin/activity/seckill/edit_time', ['id' => $item->id])?>">编辑</a>
                        <a class="btn btn-danger" data-type="del" href="<?=$this->url('./@admin/activity/seckill/delete_time', ['id' => $item->id])?>">删除</a>
                    </div>
                    <?php else:?>
                    <a class="btn btn-default" href="<?=$this->url('./@admin/activity/seckill/goods', ['act_id' => $id, 'time_id' => $item->id])?>">添加商品</a>
                    <?php endif;?>
                </td>
            </tr>
        <?php endforeach; ?>
