      ...
      <div class="j_box">
        ...
        <div class="read_btn">
          <div class="btn" style="width:328px"><a href="javascript:;" class="sc" title="收藏本书" style="margin-right:2px">加入收藏夹</a>
              <a href="" class="txt" title="<?=$book->name?>txt下载" target="_blank"><?=$book->name?>txt下载</a></div>
        </div>
        <div class="vote"><?=$book->author?>的<a href="<?=$cat->url?>"><?=$cat->real_name?></a>作品<?=$book->name?>
          ...
          <ul>
              ...
              <?php if ($key < 1):?>
                      <li><a href="<?=$item->url?>" title="<?=$item->name?>" target="_blank"><?=$item->name?></a><span>
                              <a href="<?=$item->url?>" title="<?=$item->author?>作品" target="_blank"><?=$item->author?></a></span></li>
              ...
              <?php else: ?>
                      <li><a href="<?=$item->url?>" title="<?=$item->name?>" target="_blank"><?=$item->name?></a>
                          <span><a href="<?=$item->url?>" title="<?=$item->author?>作品" target="_blank"><?=$item->author?></a></span></li>
                <?php endif;?>
              <?php endforeach;?>
          </ul>
        </div>
...
<div align="left">
<br/><h3>阅读提示：</h3><br/>
1、小说《<?=$book->name?>》所描述的内容只是作者【<?=$book->author?>】的个人写作观点，不保证其中情节的真实性，请勿模仿！<br/>
2、《<?=$book->name?>》版权归原作者【<?=$book->author?>】所有，本书仅代表作者本人的文学作品观点，仅供娱乐请莫当真。
</div>
</div>
