...
class CategoryController extends Controller {
    ...
    public function indexAction($id) {
        ...
        $new_book = BookModel::ofClassify()->isOpen()->where('cat_id', $id)->orderBy('created_at', 'desc')->limit(4)->all();
        $over_book = BookModel::ofClassify()->isOpen()->where('cat_id', $id)->where('over_at > 0')->limit(4)->all();

        $click_bang = BookModel::ofClassify()->isOpen()->where('cat_id', $id)->orderBy('click_count', 'desc')->limit(10)->all();
        $size_bang = BookModel::ofClassify()->isOpen()->where('cat_id', $id)->orderBy('size', 'desc')->limit(10)->all();
