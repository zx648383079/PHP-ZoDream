...
function bindMicroPage() {
    ...
    }).on('click', '[data-action=collect]', function() {
        let $this = $(this),
            box = $this.closest('.micro-item');
        $.getJSON(BASE_URI + 'collect', {
            id: box.data('id')
        }, function(data) {
            if (data.code == 200) {
                $this.toggleClass('active', data.data.is_collected);
            }
            parseAjax(data);
        });
    }).on('click', '[data-action=remove]', function() {
        if (!confirm('确定要删除此条微博？')) {
            return;
        }
        let $this = $(this),
            box = $this.closest('.micro-item');
        $.getJSON(BASE_URI + 'delete', {
            id: box.data('id')
        }, function(data) {
            if (data.code == 200) {
                box.remove();
