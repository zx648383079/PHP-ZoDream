...
use Zodream\Html\PageLink;

/** @var $this View */
$this->title = sprintf('“%s” 的内容列表', empty($cat) ? $model['name'] : $cat['title']);
?>
...
<div class="panel-container page-multiple-table">
    <div class="page-search-bar">
        ...
        <a class="btn btn-success" href="<?=$this->url('./@admin/content',
            empty($cat) ? ['model_id' => $model['id']] : ['cat_id' => $cat->id])?>">返回</a>
        <?php endif;?>
    ...
    <table class="table table-hover">
        ...
        <tbody>
        <?php foreach($model_list['data'] as $item):?>
            <tr>
                ...
                <td>
                    <?php if ($item['category']):?>
                        <a href="<?=$this->url('./@admin/content', ['cat_id' => $item['cat_id'], 'model_id' => $model->id])?>">
                            <?=$item['category']['title']?>
                        </a>
                    <?php else:?>
                    [未分类]
        ...
        <tfoot class="page-multiple-action">
            <tr>
                ...
                <td class="left" colspan="5">
                    <a class="btn btn-danger" data-type="del" href="<?=$this->url('./@admin/content/delete', ['id' => 0, 'cat_id' => empty($cat) ? 0 : $cat->id, 'model_id' => $model->id], false)?>">删除选中项（<span class="page-multiple-count">0</span>）</a>
                </td>
            </tr>
        </tfoot>
    </table>
    <?php if(empty($model_list['data'])):?>
    ...
    <div align="center">
        <?=PageLink::show([
            'total' => $model_list['paging']['total'],
            'pageSize' => $model_list['paging']['limit'],
            'page' => $model_list['paging']['offset']
        ])?>
    </div>
</div>
