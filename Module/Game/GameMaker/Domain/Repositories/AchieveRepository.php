...
final class AchieveRepository {
    ...
    public static function makerSave(array $data) {
        ...
        $id = $data['id'] ?? 0;
        unset($data['id']);
        $model = $id > 0 ? AchieveEntity::where('project_id', $data['project_id'])
            ->where('id', $id)->first() : new AchieveEntity();
        $model->load($data);
        if (!$model->save()) {
    ...
    public static function makerRemove(int $project, int $id) {
        ProjectRepository::isSelfOrThrow($project);
        AchieveEntity::where('project_id', $project)
            ->where('id', $id)->delete();
    }
}