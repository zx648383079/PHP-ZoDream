...
final class RecipeRepository {
    ...
    public static function makerList(int $project, string $keywords = '') {
        ...
        return RecipeEntity::query()->when(!empty($keywords), function ($query) {
    ...
    public static function makerSave(array $data) {
        ...
        $id = $data['id'] ?? 0;
        unset($data['id']);
        $model = $id > 0 ? RecipeEntity::where('project_id', $data['project_id'])
            ->where('id', $id)->first() : new RecipeEntity();
        $model->load($data);
        if (!$model->save()) {
    ...
    public static function makerRemove(int $project, int $id) {
        ProjectRepository::isSelfOrThrow($project);
        RecipeEntity::where('project_id', $project)
            ->where('id', $id)->delete();
    }
}