...
final class StoreRepository {
    public static function makerList(int $project, string $keywords = '') {
        ...
        return StoreEntity::query()->when(!empty($keywords), function ($query) {
    ...
    public static function makerSave(array $data) {
        ...
        $id = $data['id'] ?? 0;
        unset($data['id']);
        $model = $id > 0 ? StoreEntity::where('project_id', $data['project_id'])
            ->where('id', $id)->first() : new StoreEntity();
        $model->load($data);
        if (!$model->save()) {
    ...
    public static function makerRemove(int $project, int $id) {
        ProjectRepository::isSelfOrThrow($project);
        StoreEntity::where('project_id', $project)
            ->where('id', $id)->delete();
    }
}