...
var EditorEventWindowResize = 'editor_Window_resize';
...
var EditorEventDrag = 'editor_drag'; // 拖拽开始
var EditorEventDrog = 'editor_drog'; // 拖拽放下
var EditorEventOperateWeight = 'editor_operate_weight';
var EditorEventMouseMove = 'editor_mouse_move';
var EditorEventMouseUp = 'editor_mouse_up';
var EditorEventTimeLoop = 'editor_time_loop';
...
var VisualEditor = /** @class */ (function () {
    function VisualEditor(box) {
        ...
        this.weightSoul = new EditorWeightSoul(this);
    ...
    VisualEditor.prototype.reload = function () {
        this.browser.reload();
    };
    /**
     * 获取页面布局
    ...
    VisualEditor.prototype.bindEvent = function () {
        ...
        $window.on('resize', function () {
            _this.emit(EditorEventWindowResize, $window.width(), $window.height());
        });
        var bindDocEvent = function (doc, isBrowser) {
            ...
            doc.on('keydown', function (e) {
                ...
                _this.emit(EditorEventMouseMove, isBrowser ? _this.browser.globePosition(p) : p);
            }).on('mouseup', function (e) {
                var p = { x: e.clientX, y: e.clientY };
                _this.emit(EditorEventMouseUp, isBrowser ? _this.browser.globePosition(p) : p);
        ...
        this.on(EditorEventResize, function (width, height) {
            ...
            _this.emit(EditorEventResize, _this.innerWidth, _this.innerHeight);
            _this.normal();
        }).on(EditorEventWindowResize, function (ww, wh) {
            var top = _this.box.offset().top;
            var height = wh - top - 20;
            _this.box.toggleClass('visual-mobile-editor', ww < 780 && ww < wh);
            _this.outerHeight = height;
            if (!_this.viewInited) {
                return;
            }
            _this.emit(EditorEventResize, _this.innerWidth, _this.innerHeight);
        }).on(EditorEventMouseMove, function (p) {
            if (_this.mouseListener.move) {
                _this.mouseListener.move(p);
            }
        }).on(EditorEventMouseUp, function (p) {
            if (_this.mouseListener.finish) {
                _this.mouseListener.finish(p);
            }
        }).on(EditorEventBrowserReady, function (doc) {
    ...
    VisualEditor.prototype.loopCheckBox = function () {
        ...
        var checkFunc = function () {
            var now = new Date();
            _this.emit(EditorEventTimeLoop, now);
...
var EditorWeightSoul = /** @class */ (function () {
    function EditorWeightSoul(editor) {
        var _this = this;
        this.editor = editor;
        this.isNew = false;
        this.editor.on(EditorEventViewInit, function () {
            _this.box = _this.editor.find('.await-widget-box');
        }).on(EditorEventDrag, function (element, isNew, mousePos) {
            ...
            _this.isNew = isNew;
            _this.box.html(element.prop('outerHTML'));
            _this.editor.onMouse(function (p) {
                if (!_this.target) {
                    return;
                }
                _this.setBound(p.x - mouseInPos.x, p.y - mouseInPos.y, width, height);
            }, function (p) {
                var _a;
                if (!_this.target) {
                    return;
                }
                var items = _this.editor.browser.getElementByPoint(p);
                if (!items[0]) {
                    _this.reset();
                    return;
                }
                // 在手机端 touchend 事件出现问题无法被部件捕捉到
                (_a = _this.editor).emit.apply(_a, __spreadArray([EditorEventDrog], items, false));
            });
        }).on(EditorEventDrog, function (row, replace) {
            if (!_this.target) {
                return;
            }
            _this.editor.emit(EditorEventOperateWeight, _this.isNew, _this.target, row, replace);
            _this.reset();
        });
    }
    EditorWeightSoul.prototype.setBound = function (x, y, width, height) {
        this.box.css({
            width: width + 'px',
            height: height + 'px',
            left: x + 'px',
    ...
    EditorWeightSoul.prototype.reset = function () {
        if (!this.target) {
            return;
        }
...
var EditorPanelGroup = /** @class */ (function () {
    function EditorPanelGroup(editor) {
        ...
        this.editor.on(EditorEventViewInit, function () {
            ...
            var maxHeight = _this.box.height();
...
var EditorPropertyPanel = /** @class */ (function () {
    function EditorPropertyPanel(editor) {
        ...
        this.editor.on(EditorEventAfterViewInit, function () {
            _this.bindEvent();
        }).on(EditorEventGetStyleSuccess, function (data) {
            ...
            _this.editor.emit(EditorEventGetWeightProperty, weight.id(), function (data) {
                _this.lastAt = new Date().getTime();
                _this.applyForm(data);
            });
        }).on(EditorEventTimeLoop, function (now) {
            if (!_this.isAsync) {
                return;
            }
            var space = 20000;
            var nowTime = now.getTime();
            if (nowTime - _this.lastAt > space) {
                _this.saveSync();
                _this.lastAt = nowTime;
            }
        });
    }
    EditorPropertyPanel.prototype.bindEvent = function () {
        var _this = this;
        if (!this.box) {
            return;
        }
        ...
        this.box.on('change', '.position-input select', function () {
            var $this = $(this);
            $this.next().html(EditorHtmlHelper.positionSide($this.val()));
        }).on('click', '.background-input input[type="radio"]', function () {
            var $this = $(this);
            $this.closest('.background-input').find('.value-input').html(EditorHtmlHelper.backgroundValue($this.attr('name').replace('[type]', ''), $this.val()));
    ...
    EditorPropertyPanel.prototype.saveSync = function () {
        var _this = this;
        this.isAsync = false;
        if (!this.target) {
            return;
        }
        var data = formData(this.box.find('.form-table')) + '&theme_style_id=' + EditorHelper.parseNumber(this.box.find('.style-item.active').attr('data-id')) + '&id=' + this.target.id();
        this.editor.emit(EditorEventSaveWeightProperty, this.target.id(), data, function (data) {
            _this.target.html(data.html);
        });
    };
    EditorPropertyPanel.prototype.autoSave = function () {
        this.isAsync = true;
    ...
    EditorPropertyPanel.prototype.applyForm = function (data) {
        ...
        items[0].innerHTML = EditorHtmlHelper.title(data.title)
            + EditorHtmlHelper.lazy((_b = data.settings) === null || _b === void 0 ? void 0 : _b.lazy)
...
var EditorWeightPanel = /** @class */ (function () {
    ...
    EditorWeightPanel.prototype.bindEvent = function () {
    ...
    function EditorDialog(editor) {
        ...
        this.editor.on(EditorEventViewInit, function () {
            ...
            _this.box.on('done', function () {
                _this.editor.emit(EditorEventSaveWeightProperty, _this.target.id(), target.find('.dialog-body').serialize(), function (data) {
...
var EditorBrowser = /** @class */ (function () {
    function EditorBrowser(editor) {
        ...
        this.editor = editor;
        this.frameScale = 100;
        this.shellVisbile = false;
        this.editor.on(EditorEventViewInit, function () {
            _this.shellTimeBar = _this.editor.find('.shell-bar .time');
            ...
            if (!_this.shellVisbile) {
                return;
            }
            _this.shellTimeBar.text([now.getHours(), now.getMinutes()].map(EditorHelper.twoPad).join(':'));
        }).on(EditorEventOperateWeight, function (isNew, weight, row, replace) {
            _this.dragWeight = undefined;
            if (isNew) {
                _this.selectedWeight = new EditorWeight(_this.editor, weight.clone());
                _this.selectedWeight.appendTo(row, replace);
                return;
            }
            _this.selectedWeight = new EditorWeight(_this.editor, weight);
            _this.selectedWeight.moveTo(row, replace);
        });
    }
    EditorBrowser.prototype.bindEvent = function () {
        var _this = this;
        this.frame.on('load', function () {
            var $doc = _this.frame.contents();
            _this.frameBody = $doc.find('body');
            _this.editor.emit(EditorEventBrowserReady, $doc);
            _this.bindFrameEvent();
        });
    };
    EditorBrowser.prototype.bindFrameEvent = function () {
        var that = this;
        var drogFunc = function (row, replace) {
            if (that.dragWeight) {
        ...
        this.frameBody.on('dragover', '.visual-edit-row', function (e) {
            e.stopPropagation();
            e.preventDefault();
            ...
            var row = $(this);
            that.editor.emit(EditorEventDrog, row.closest('.visual-edit-row'), row);
        }).on('dragover', '.visual-edit-control', function (e) {
            e.stopPropagation();
            e.preventDefault();
            ...
            var weight = $(this).closest('.visual-edit-control');
    ...
    EditorBrowser.prototype.globePosition = function (p) {
    ...
    EditorBrowser.prototype.framePosition = function (p) {
        var offset = this.frame.offset();
        return {
            x: p.x - offset.left,
            y: p.y - offset.top
    ...
    EditorBrowser.prototype.getElementByPoint = function (p) {
        var pos = this.framePosition(p);
        if (pos.x < 0
            || pos.y < 0
    ...
    EditorBrowser.prototype.toggleShell = function (visible) {
    ...
    */
    ...
    EditorBrowser.prototype.reload = function () {
    ...
    EditorBrowser.prototype.serialize = function () {
        ...
        var data = [];
        this.frameBody.find('.visual-edit-row').each(function () {
            var row = $(this);
...
var EditorHtmlHelper = /** @class */ (function () {
    ...
    EditorHtmlHelper.value = function (val, def) {
        if (def === void 0) { def = ''; }
    ...
    EditorHtmlHelper.join = function () {
        var items = [];
        for (var _i = 0; _i < arguments.length; _i++) {
            items[_i] = arguments[_i];
        }
        return items.map(function (val) { return EditorHtmlHelper.value(val); }).join('');
    };
    ...
    EditorHtmlHelper.share = function (val) {
        var id = 'is_share_' + EditorHtmlHelper.guid();
    ...
    EditorHtmlHelper.border = function (name, data) {
        ...
        return EditorHtmlHelper.input(id, '边框', EditorHtmlHelper.join('<input type="text" class="form-control" name="', name, '[value][]" value="', data === null || data === void 0 ? void 0 : data.value[0], '" placeholder="粗细" size="4"><select name="', name, '[value][]">', option, '</select><input type="color" name="', name, '[value][]" value="', data === null || data === void 0 ? void 0 : data.value[2], '"><div class="side-input">', html, '</div>'));
    };
    ...
    EditorHtmlHelper.color = function (name, data) {
        ...
        var id = EditorHtmlHelper.nameToId(name) + '_' + EditorHtmlHelper.guid();
        return EditorHtmlHelper.input(id, '字体颜色', EditorHtmlHelper.join(EditorHtmlHelper.radio(id, name + '[type]', ['无', '有'], data === null || data === void 0 ? void 0 : data.type), '<input type="color" name="', name, '[value]" value="', data === null || data === void 0 ? void 0 : data.value, '">'));
    };
    ...
    EditorHtmlHelper.switch = function (id, name, val, onLabel, offLabel) {
        if (onLabel === void 0) { onLabel = '开启'; }
        if (offLabel === void 0) { offLabel = '关闭'; }
    ...
    EditorHtmlHelper.button = function (text, cls) {
        return "<button type=\"button\" class=\"btn ".concat(cls, "\">").concat(text, "</button>");
    };
