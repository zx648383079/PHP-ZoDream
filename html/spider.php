    ...
    while ($page > 0) {
        try {
            $data = (new Curl('https://www.youzan.com/v2/trade/order/list.json?p='.
                ...
                ->setReferrer('https://www.youzan.com/v2/weixin/autoreply/scan')
                ->setCookie('gr_user_id=18995ddb-0101-4e35-8676-2d05eff05d71; KDTSESSIONID=c48ef006902be59042a610e02c; UM_distinctid=15ad52c21ca61d-066295d37df7a6-67f1a39-100200-15ad52c21cbc2c; _canwebp=1; _kdt_id_=18129825; redirect=https%3A%2F%2Fwww.youzan.com%2Fv2%2Fweixin%2Fautoreply%2Fscan%3F; captcha_response=9090809fd2f18a749e1ab97558717c2144ef2667; avatar=https%3A%2F%2Fdn-kdt-img.qbox.me%2Fupload_files%2Favatar.png; mobile=13917861882; kdt_id=18129825; team_auth_key=af3b450e570c1d102fa690d8a7045f35; weixin_server_key=9621b2a606ec67df91f5a76afc7e0f68; weixin_oldsub_key=4c7b93460b460f22f5ffe9f1ab8dc15d; weixin_certsub_key=6fddb1954989ad38eed854cf67d038ba; weixin_menu_key=45fdf77f27a2213a252c1707146312a5; mp_id=775558; weixin_subscribe_key=ac786ecd65ccf6cab61eca1273a7deab; sinaweibo_id=18129825; sinaweibo_uid=5837662734; weibo_auth_key=c7058b0de766df818b65bc11da3e4cc3; Hm_lvt_7bec91b798a11b6f175b22039b5e7bdd=1490146980,1490254535,1490789974,1491970040; Hm_lpvt_7bec91b798a11b6f175b22039b5e7bdd=1491970320; sid=9bae244647205fa3910eeb09e7affc60; sKey=Nxj5Kjadiebrv5FbsjyPgATbffTQ8H7f8TbIqdKrsnY%3D; user_id=782990; user_weixin=%2B86-13917861882; user_nickname=ykoko; kdtnote_fans_id=0; _canwebp=1; Hm_lvt_7fff7ceede91c07fb0a2f9c1850d7987=1490147031,1490254540,1490789997,1491970065; Hm_lpvt_7fff7ceede91c07fb0a2f9c1850d7987=1491970332; gr_session_id_767813e963734402a8256e1096b88331=5a6e6e08-c005-4fc9-9db7-e4d8dc9d9c82; gr_cs1_5a6e6e08-c005-4fc9-9db7-e4d8dc9d9c82=user_id%3A782990')
                ->setHeader([
                    'User-Agent' => 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/57.0.2987.110 Safari/537.36',
                    'Accept' => 'application/json, text/javascript, **; q=0.01'
                ])->get();
            (new File(__DIR__.'/json/oo_'.$page.'.json'))
                ->write($data);
            echo $page,' GOOD <br/>';
        } catch (Exception $ex) {
            echo $page,'<br/>';
...
function saveOrder($data) {
    ...
    foreach ($data as $item) {
        $order_status = 1;
        $shipping_status = 0;
        $pay_status = $item['pay_state'];
        if ($item['order_state'] == 'cancel') {
            $order_status = 2;
        } else if ($item['order_state'] == 'sign') {
            $shipping_status = 2;
            $pay_status = 1;
        }
        ...
        $id = (new \Zodream\Infrastructure\Database\Query\Record())
            ...
            ->set([
                ...
                'tel' => $item['tel'],
                'pay_name' => $item['buy_way_str'],
                'pay_time' => strtotime($item['pay_time']),
                'remark' => $item['remark'],
                'postscript' => $item['buyer_msg'],
                'goods_amount' => $item['pay']
            ])->insert();
        foreach ($item['items'] as $goods) {
            (new \Zodream\Infrastructure\Database\Query\Record())
                ->setTable('order_goods')
                ->set([
                    'order_id' => $id,
                    'goods_id' => $goods['sku_id'],
                    'goods_name' => $goods['goods_info']['title'],
                    'goods_number' => $goods['num'],
                    'goods_price' => $goods['pay_price'],
                    'discount_fee' => $goods['postage']
                ])->insert();
        }
    }
}
...
set_time_limit(0);
for ($i = 1; $i < 11; $i ++) {
    $json = (new File(__DIR__.'/json/oo_'.$i.'.json'))->read();
    $json = json_decode($json, true);
    saveOrder($json['data']['list']);
    echo 'GOOD<BR>';
}