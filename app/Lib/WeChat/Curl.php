...
class Curl {
     ...
     * @return bool|mixed
     */
    public static function request($queryUrl, $param = null, $method = 'get', $is_json = true, $is_urlcode = true) {
        if (empty($queryUrl)) {
            return false;
        }
    ...
    public static function get($url, $isJson = TRUE) {
    	return self::request($url, null, 'GET', $isJson);
    }
    
    public static function post($url, $param = null, $isJson = TRUE, $urlCode = TRUE) {
    	return self::request($url, $param, 'POST', $isJson, $urlCode);
    }
